...
export default function SharePage() {
	...
	const titleFromQuery = params?.get("title") || "Check this out";

	const [copied, setCopied] = useState<string | null>(null);

	const currentUrl = typeof window !== "undefined" ? window.location.origin + (window.location.pathname || "") : "";
	const shareTitle = titleFromQuery;
	...
	const actions: Action[] = useMemo(
		() => [
			...
			},
			{
				key: "message",
				label: "Message",
				onClick: () => open(`sms:?&body=${encodeURIComponent(`${shareTitle} ${currentUrl}`)}`),
				icon: (
			...
			},
			{
				...
				onClick: () =>
					open(
						`mailto:?subject=${encodeURIComponent(shareTitle)}&body=${encodeURIComponent(currentUrl)}`
					),
	...
	return (
		<div className="fixed inset-0 z-50 flex items-end justify-center">
			...
			>
				...
				<div className="grid grid-cols-4 gap-3">
					{actions.map((a) => (
						...
						>
							...
							<span className="text-[12px] leading-5 text-[#111111] font-medium text-center">
								{a.label}
							</span>
						</button>
					))}
				...
				{/* Copy status */}
				{copied && (
					<div className="mt-4 text-center text-[12px] text-[#78828A]">{copied}</div>
				)}
			</section>
		</div>
	);
